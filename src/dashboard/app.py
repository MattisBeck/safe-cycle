...
def generate_map() -> str:
    ...
    ">
        <h4 style="margin: 0 0 5px 0; color: #111827; font-size: 12px;">
            Gefahren-Heatmap
        </h4>
        <div style="
            background: linear-gradient(
                to right, blue, cyan, lime, yellow, red
            );
            width: 100%; height: 8px; border-radius: 2px; margin-bottom: 3px;
        "></div>
        <div style="
            display: flex; justify-content: space-between;
            font-size: 9px; color: #4b5563; font-weight: 600;
        ">
            <span>Geringer</span>
            <span>Hoch</span>
        </div>
        <ul style="
            margin: 5px 0 0 0; padding-left: 15px;
            font-size: 10px; color: #374151;
        ">
            <li style="margin-bottom: 2px;">
                <strong>Blau/Grün:</strong> &gt; 1.5m Abstand
            </li>
            <li>
                <strong>Rot/Gelb:</strong> &lt; 1.5m oder schnell
            </li>
        </ul>
        <hr style="border: 0; border-top: 1px solid #e5e7eb; margin: 6px 0;">
        <p style="font-size: 9px; margin: 0; color: #6b7280; line-height: 1.2;">
            <i><b>Tipp:</b> Mehrere Vorfälle am selben Ort verstärken das
            Rot ebenfalls.</i>
        </p>
    </div>
    """
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        """Initialisiert den Dashboard-Handler."""
        super().__init__(*args, directory=str(STATIC_DIR), **kwargs)  # type: ignore
